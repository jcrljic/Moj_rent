<!DOCTYPE html>
<html lang="hr">
	<html>
	<link rel="stylesheet" href="css/style.css" type="text/css">
		<body>
		<div id="header">
		<div class="clearfix">
			<div class="logo">
				<a href="index.html"><img src="images/preuzmi (4).jpg" alt="LOGO" height="100" width="362"></a>
			</div>
			<ul class="navigation">
				<li>
					<a href="index.html">Početna</a>
				</li>
				<li> 
					<a href="about.html">O nama</a>
				</li>
				<li>
					<a href="news.html">Novosti</a>
				</li>
				<li>
					<a href="narudzbe.html">Online narudžbe</a>
				</li>
				<li>
					<a href="kontakt.html">Kontakt</a>
				</li>
			</ul>
		</div>
	</div>
			<meta charset="UTF-8">
		</header>
			<h1>Recite nam što mislite o našoj usluzi</h1>
<form>

	<form name="dojmovnik" action="putanja_do_php_skripte" method="POST">
		<label for="name">Ime:</label>
		<br>
		<input id="name" type="text" name="ime"/>
			<br></br>
		<label for="surname">Prezime:</label>
		<br>
		<input id="surname" type="text" surname="prezime"/>
		<br></br>
		<label for="name">Grad:</label>
		<br>
		<input id="name" type="text" name="Grad"/>
		<br></br>
		<label for="name">Država:</label>
		<br>
		<input id="name" type="text" name="Država"/>
		<br></br>
		<label for="name">E-mail:</label>
		<br>
		<input id="name" type="text" name="e-mail"/>
		<br></br>
		<label for="name">Komentar:</label>
		<br>
		<textarea name="Unos" rows="10" cols="20"></textarea>
		
		<br></br>
		<br></br>
	<input type="submit" value="Pošalji">
</form>
	<div id="footer">
		<div class="clearfix">
			<div class="section">
				<h4>Posljednje vijesti</h4>
				<p>
					U našim novostima pogledajte posebne pogodnosti za naše vjerne korisnike
				</p>
			</div>
			<div class="section contact">
				<h4>Kontaktirajte nas:</h4>
				<p>
					<span>Adresa:</span> Dubrava
				</p>
				<p>
					<span>Telefon:</span> 01/123-4567
				</p>
				<p>
					<span>Email:</span> info_auti.rent@gmail.com
				</p>
			</div>
			<div class="section">
				<h4>POŠALJITE NAM PORUKU</h4>
				<p>
					Ako imate problema sa korištenjem ove stranice ne oklijevajte nego pitaj te za pomoć na forumu
				</p>
				<a href="dojmovnik.html" class="subscribe">Stisni te za slanje email-a</a>
			</div>
		</div>
		<div id="footnote">
			<div class="clearfix">
				<div class="connect">
					<a href="http://freewebsitetemplates.com/go/facebook/" class="facebook"></a><a href="http://freewebsitetemplates.com/go/twitter/" class="twitter"></a><a href="http://freewebsitetemplates.com/go/googleplus/" class="googleplus"></a><a href="http://pinterest.com/fwtemplates/" class="pinterest"></a>
				</div>
				<p>
					© Copyright 2021. VVG-IS. All Rights Reserved.
				</p>
			</div>
		</div>
	</div>
</body>
</html>
</body>
</html>